...
public class SqlWriter implements Piped<TransformedChange> {
     ...
     * the database. */
    @Override
    public void send(final TransformedChange input) {
        try {
            final Row row = input.getRow();
            if (row.isDeleted()) {
    ...
    private void deleteCouchId(final String id) throws SQLException {
        deleteCouchIdStatement.setString(1, id);
        deleteCouchIdStatement.execute();
        final ResultSet rs = deleteCouchIdStatement.getResultSet();
        if (!rs.next()) {
     ...
     * document. */
    private void deleteDimensionTables(final JsonNode dimensions)
        throws SQLException {
        if (dimensions == null) {
            return;
        }
    ...
    private void insertDimensionTables(final JsonNode dimensions)
        throws SQLException {
        if (dimensions == null) {
            return;
        }
    ...
    private void upsertTargetTable(final JsonNode input) throws SQLException {
    ...
    private void updateSequence(final String seq) throws SQLException {
        updateSeqStatement.setString(1, seq);
        updateSeqStatement.execute();
        final ResultSet rs = updateSeqStatement.getResultSet();
        if (!rs.next()) {
    ...
    private PreparedStatement makeInsertStatement(
        final String table, final JsonNode data)
        throws SQLException {
